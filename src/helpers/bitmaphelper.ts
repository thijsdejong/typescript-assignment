...
export class BitmapHelper {
    ...
    for (let y = 0; y < bitmap.getHeight(); y++) {
      ...
      for (let x = 0; x < bitmap.getWidth(); x++) {
        ...
        if (bitmap.getPixel(x, y).getValue() === 0) {
          newValue = this.getDistanceToClosestWhitePixelByBitmapAndPixel(
            bitmap,
            x,
            y
          );
  ...
  private static getDistanceToClosestWhitePixelByBitmapAndPixel(
    bitmap: Bitmap,
    pixelX: number,
    pixelY: number
  ): number {
    ...
    for (let targetY = 0; targetY < bitmap.getHeight(); targetY++) {
      for (let targetX = 0; targetX < bitmap.getWidth(); targetX++) {
        ...
        distance = Math.min(
          distance,
          this.calculateDistance(targetX, pixelX, targetY, pixelY)
        );
      }
    }
